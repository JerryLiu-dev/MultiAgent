...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        res = 0
        # rewarded when food eaten, else decrement distance from nearest food
        if len(newFood.asList()) < len(currentGameState.getFood().asList()):
            res += 100
        else:
            dists = []
            for food in newFood.asList():
                dists.append(util.manhattanDistance(food,newPos))
            
            if len(dists) > 0:
                res -= min(dists)

        # decrement when meet ghost
        if newGhostStates[0].getPosition() == newPos:
            res -= 1000

        # reward when eat scare pellet    
...
def betterEvaluationFunction(currentGameState: GameState):
    ...
    if currentGameState.isLose():
        return -1111
    currPos = currentGameState.getPacmanState().getPosition()
    foods = currentGameState.getFood().asList()
    dists = []
    for food in foods:
        dists.append(util.manhattanDistance(food,currPos))
        if food == currPos:
            res += 1000
    ...
    res -= len(foods)*20
    res -= min(dists)
    res -= len(currentGameState.getCapsules())
    ghosts = currentGameState.getGhostStates()
    for ghost in ghosts:
        if ghost.getPosition() == currPos:
            res -= 1000
